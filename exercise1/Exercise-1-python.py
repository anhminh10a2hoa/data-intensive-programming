...
def myTripleSum(a: int, b: int, c: int) -> int:
    return a + b + c
...
cubeSum: int = reduce(lambda x, y: x + y, map(lambda x: x ** 3, myList))

largestValue: int = reduce(lambda x, y: x if x > y else y, map(lambda x: 1 + 9 * x - x ** 2, myList))
